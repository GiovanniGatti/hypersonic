...
public final class Contest implements Callable<Contest.ContestResult> {
    ...
    private final List<Function<InputSupplier, Supplier<AI>>> ais;
    ...
    public Contest(
            List<Function<InputSupplier, Supplier<AI>>> ais,
    ...
    public ContestResult call() throws InterruptedException, ExecutionException {
        ...
        for (Supplier<GameEngine> gameEngine : gameEngines) {
            ...
            for (int i = 0; i < ais.size() - 1; i++) {
                Function<InputSupplier, Supplier<AI>> player = ais.get(i);
                for (int j = i + 1; j < ais.size(); j++) {
                    Function<InputSupplier, Supplier<AI>> opponent = ais.get(j);
                    games.add(
                            new Game(
                                    player,
                                    opponent,
